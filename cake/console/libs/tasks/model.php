...
//ADD `active` BOOLEAN NULL DEFAULT TRUE, 
//ADD `created` DATETIME NULL DEFAULT NULL ,
//ADD `created_by` INT NULL DEFAULT NULL , 
//ADD `modified` DATETIME NULL DEFAULT NULL,
//ADD `modified_by`  INT NULL DEFAULT NULL ;");
                        $fields = $object->schema(true);      
                        $tmp_display_field_name = Inflector::underscore($modelClass);
                        $displayField = $object->hasField(array('name', 'title', strtolower("{$tmp_display_field_name}_name")));
                        $all = 'all';
                        if(!empty($displayField)) {
                            $vars = compact('displayField', 'fields', 'all');
                        } else {
                            $vars = compact('fields', 'all'); 
                        }
			if ($this->bake($object, $vars) && $unitTestExists) {
	...
	function __interactive() {
		...
		if (in_array($useTable, $this->_tables)) {
			...
			return false;
                ...
                $tmp_display_field_name = Inflector::underscore($currentModelName);
		$displayField = $tempModel->hasField(array('name', 'title', strtolower("{$tmp_display_field_name}_name")));
		if (!$displayField) {
		...
		if (strtolower($looksGood) == 'y') {
			$vars = compact('associations', 'validate', 'primaryKey', 'useTable', 'displayField', 'fields');
	...
	function fieldValidation($fieldName, $metaData, $primaryKey = 'id') {
		...
		while ($anotherValidator == 'y') {
			...
			if ($metaData['null'] != 1 && !in_array($fieldName, array($primaryKey, 'created', 'modified', 'updated'))) {
				if ($fieldName == 'email' || strpos($fieldName, 'email_') !== false) {
					$guess = $methods['email'];
			...
			if (isset($this->_validations[$choice])) {
				...
				$validatorName = Inflector::slug($choice);
                        ...
                        if (isset($metaData['key']) && $metaData['key'] == 'unique' && !in_array($fieldName, array($primaryKey, 'created', 'modified', 'updated'))) {
 ...
 */
	function bake($name, $data = array()) {
		if (is_object($name)) {
			if ($data == false || !empty($data['all']) == 'all') {
				if(empty($data)) {
                                    $data = array();
                                }
                                $associations = array();
