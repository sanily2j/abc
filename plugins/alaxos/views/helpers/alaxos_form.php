...
class AlaxosFormHelper extends FormHelper
{
	...
	{
	    if(empty($attributes['options']) && strtolower($fieldname) == 'active')
	    {
	        $attributes['options'] = array('1' => __d('alaxos', 'Active', true), '0' => __d('alaxos', 'Inactive', true));
	    }
	...
	function filter_select($fieldname, $attributes = array())
	{
	    ...
	    $selected            = !empty($attributes['selected']) ? $attributes['selected'] : null;
            if(isset($attributes['empty']) && $attributes['empty'] !== false) {
                ...
                $attributes['empty'] = !empty($attributes['empty'])    ? $attributes['empty']    : 'All ' . Inflector :: pluralize(preg_replace('/_id$/', '', $fieldname));
            }
	    if (strtolower($attributes['empty']) == 'all actives') {
                $attributes['empty'] = 'All';
            }
            $attributes['empty'] = str_replace('_', ' ', $attributes['empty']);
	...
	public function input($fieldname, $options = array())
	{
	    ...
	    $fieldtype = !empty($options['type']) ? $options['type'] : null;
            ...
            if (empty($fieldtype) && strpos(strtolower($fieldname), 'email') !== false) {
                $fieldtype = 'email';                
            }
            if (empty($fieldtype) && strtolower($fieldname) == 'active') {
                $fieldtype = 'active';                
            }
            
            if (empty($fieldtype) && strpos(strtolower($fieldname), 'file_') !== false) {
                $fieldtype = 'file';                
            }
            
            if (empty($fieldtype) && strpos(strtolower($fieldname), 'pincode') !== false) {
                $fieldtype = 'pincode';                
            }
            
            if (empty($fieldtype) && strpos(strtolower($fieldname), 'fax') !== false) {
                $fieldtype = 'fax';                
            }
            
            if (empty($fieldtype) && strpos(strtolower($fieldname), 'phone') !== false) {
                $fieldtype = 'phone';
            }
	    if(!isset($fieldtype))
	    {
	    ...
	    switch($fieldtype)
	    {
	        ...
	        case 'datetime':
	            ...
	            break;
	        case 'active':
	            return $this->_get_label($fieldname, $options) . $this->input_active_inactive($fieldname, $options);
	            break;
	        ...
	        case 'int':
	            ...
	            break;
                case 'pincode':
	            return $this->_get_label($fieldname, $options) . $this->input_pincode($fieldname, $options);
	            break;
                case 'fax':
                case 'phone':
	            return $this->_get_label($fieldname, $options) . $this->input_phone($fieldname, $options);
	            break;
	        case 'email':
	            return $this->_get_label($fieldname, $options) . $this->input_email($fieldname, $options);
	            break;    
	        case 'file':
	            return $this->input_file($fieldname, $options);
	            break;    
	        default:
	...
	public function input_pincode($fieldname, $options = array())
	{
            $options['maxlength'] = '6';
            return $this->input_number($fieldname, $options);	    
	}
        
	public function input_phone($fieldname, $options = array())
	{
            $options['type'] = 'tel';
	...
	public function get_download_link($file_name, $table_name, $id, $field_name)
        {
            // if saving validation fails
            if (is_array($file_name)) {
                return;
            }
            $file_name_without_ext = substr($file_name, 0, strrpos($file_name, '.'));
            $file_ext = end(explode('.', $file_name));
            return '&nbsp;<a href="/' . $table_name .'/download/' . "$file_name_without_ext/$file_ext/" . base64_encode("/uploads/$table_name/$field_name/$id/") . '">' . $file_name . '&nbsp;<img src="/alaxos/img/toolbar/download_file.png" alt="Download" /></a>';
        }
	public function input_file($fieldname, $options = array())
	{   
            $op = $this->file($fieldname, $options) ;
            $model_field = explode('.', $fieldname);                        
            if(!empty($this->data[$model_field[0]]['id'])) {
                $value = $this->data[$model_field[0]][$model_field[1]];
                $id = $this->data[$model_field[0]]['id'];
                if (!empty($value)) {
                    $tmpOption = array('checked' => false, 'value' => $value);
                    $table_name = Inflector::tableize($model_field[0]);
                    $op .= $this->get_download_link($value, $table_name, $id);
                    $op .= '&nbsp;&nbsp;<span class="delete_file">Delete attached file&nbsp;:&nbsp;' . $this->checkbox($fieldname . '_del', array_merge($options, $tmpOption)) . '</span>';
                }
            }
            return $op;
	}

	public function input_email($fieldname, $options = array())
	{	    	    	    
            $options['class'] = 'inputEmail';
            $options['type'] = 'email';
            $input_field = $this->text($fieldname, $options);            
            return $input_field;
	}
...
//		$options['readonly'] = 'true';
	...
	/**
	 * Wrapper method for the FormHelper->input_active_inactive() method called automatically with 'Active' and 'Inactive' options
	 *
	 * @param string $fieldname
	 * @param array $options
	 * @param mixed $selected
	 * @param array $attributes
	 */
	public function input_active_inactive($fieldname, $options = array(), $selected = null, $attributes = array())
	{
	    $options = array('1' => __d('alaxos', 'Active', true), '0' => __d('alaxos', 'Inactive', true),);	    
	    $attributes = array_merge(array('label' => false, 'empty' => false), $attributes);
	    return $this->select($fieldname, $options, $selected, $attributes);
	}
	 ...
	 * @param array $options
	 */
	public function input_actions_list_without_delete($fieldName = '_Tech.action', $options = array())
	{
	    $options['id'] = !empty($options['id'])      ? $options['id']      : 'ActionToPerform';
	    $actions       = !empty($options['actions']) ? $options['actions'] : array(
//                'deleteAll' => ___d('alaxos', 'delete all', true),
                'activateAll' => ___d('alaxos', 'activate all', true),
                'deactivateAll' => ___d('alaxos', 'deactivate all', true),
                );
